...
use quote::quote;
use syn::{Field, Ident, ItemStruct, Visibility};
...
pub fn firefly(args: TokenStream, item: TokenStream) -> TokenStream {
                        ...
                        &mut fields
                            ...
                            .filter_map(|v| match (&v[0], &v[1], &v[2]) {
                                (
                                    ...
                                    if &ident.to_string() == "owned_by" && punct.as_char() == '=' {
    ...
    if args.read {
        ...
        for owned_by in owned_by_fields {
            extra_impls.push(_has_owner(
                ident.clone(),
                &owned_by.0.ident.unwrap(),
                &owned_by.1,
            ));
        }
    ...
    if args.from_context {
        extra_impls.push(_from_context(ident.clone()));
    }
...
fn _has_owner(
    name: Ident,
    owned_by_field: &Ident,
    _owned_by_ty: &Ident,
) -> proc_macro2::TokenStream {
    quote! {
        #[async_trait::async_trait]
        impl HasOwnerTrait<#name> for #name
        {
            async fn check_owner(self, owner: &(impl HasId + Send + Sync)) -> Result<#name, Box<dyn std::error::Error>> {
...
fn _from_route(name: Ident, id_field: &Field) -> proc_macro2::TokenStream {
    let id_field = id_field.clone();
    let id_field_name = id_field.ident.unwrap().to_string();

    quote! {
        #[async_trait::async_trait]
        impl FromRouteTrait for #name {
            async fn from_route(ctx: &Ctx) -> Result<Self, Box<dyn std::error::Error>> {
                let id = Uuid::from_str(&ctx.params().get(#id_field_name).unwrap().param).unwrap();
                let db: &Pool = ctx.extra.get();

                Ok(Self::read(&db.get().await.unwrap(), &id)
                   .await
                   .unwrap())
            }
        }
